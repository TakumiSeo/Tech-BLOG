Title: Azure SRE Agent GA 後の整理ノート（権限・実行モード・拡張・PoC 評価）
Date: 2026-03-13
Modified: 2026-04-03
...
Summary: Azure SRE Agent GA 後の Microsoft Learn ドキュメントを章別に再精査し、アクセス制御・実行モード・拡張機能・PoC 評価フレームワークを整理したノート。


本稿は、2026-04-03 時点の Microsoft Learn に公開されている Azure SRE Agent 関連ドキュメントを章単位で再精査し、GA 後の事実だけに絞って再構成したノートです。

> 初版 (2026-03-13) からの主な更新:
>
> - 2026-03-28 付で更新された docs に合わせ、アクセス制御を「4レイヤー」から「**3レイヤー**」に修正（OBO はエージェント権限レイヤーの一部として整理）。
> - Run modes の既定値を最新 docs に合わせて修正（response plan・scheduled task ともに既定 Autonomous、エージェントレベル既定は Review）。
> - 全章のリファレンス URL を再検証。
...
3. アクセス制御の 3 レイヤー
4. エージェント権限と OBO
5. Run modes（Review / Autonomous）
6. Scheduled tasks
7. Incident platforms と response plans
8. Memory system
...
- [0-3] https://learn.microsoft.com/ja-jp/azure/sre-agent/user-roles — 「アクセス制御は、3 つのレイヤーで機能」
...
## 3. アクセス制御は「3レイヤー」で整理する

### 説明
最新の docs（2026-03-28 更新）では、アクセス制御は **3 レイヤー**で説明されています。[3-1]

| レイヤー | 何を制御するか | 設定場所 |
| --- | --- | --- |
| ユーザー ロール | エージェント上でユーザーが何をできるか | エージェントリソースの Azure IAM |
| 実行モード | エージェントが実行前に承認を求めるか | response plan / scheduled task ごと |
| エージェント権限 | エージェントが Azure 上で何にアクセスできるか（OBO フォールバック含む） | リソースグループの RBAC ロール |

> **初版からの変更**: 旧稿では OBO を独立した第4レイヤーとしていましたが、最新 docs ではエージェント権限レイヤーの一部（Managed Identity RBAC + OBO fallback）として統合されています。[3-1][3-2]
...
| ロール | できること | できないこと |
| --- | --- | --- |
| SRE Agent Reader | スレッド、ログ、インシデントの閲覧 | チャット、アクション要求、設定変更 |
| SRE Agent Standard User | チャット、診断、アクション要求 | アクション承認、コネクタ管理、削除 |
| SRE Agent Administrator | アクション承認、コネクタ管理、設定変更、削除 | — |

エージェント作成者には自動で Administrator が付与されます。[3-4]

### ポータルの権限強制
ポータルは 2 段階で権限を強制します。[3-5]

1. **レベル1**: ロール未割り当てなら Access Required 画面を表示
2. **レベル2**: ロールがあっても権限外の操作はバックエンドが 403 で拒否

> UI 上でボタンが表示される場合があっても、バックエンドは正しく止めると明記されています。ただし、この UI 側の無効化はまだ全機能で一貫していないとの注記もあります。[3-5]

### スライド要点
- 「誰が押せるか」と「エージェントが何を実行できるか」は別問題として分けて説明する。[3-1]
- Standard User は診断や提案まで、Administrator は承認と構成変更まで担当する設計が基本線。[3-3]
- ロール割り当ては Azure CLI でも可能。`az role assignment create --role "SRE Agent Standard User"` [3-6]

### 参考（第3章）
- [3-1] https://learn.microsoft.com/en-us/azure/sre-agent/user-roles — 「Access control works across three layers」
- [3-2] https://learn.microsoft.com/en-us/azure/sre-agent/user-roles — 「User roles / Run modes / Agent permissions」
- [3-3] https://learn.microsoft.com/en-us/azure/sre-agent/user-roles — 「SRE Agent Reader / Standard User / Administrator」
- [3-4] https://learn.microsoft.com/en-us/azure/sre-agent/user-roles — 「The user who creates the agent automatically gets the SRE Agent Administrator role」
- [3-5] https://learn.microsoft.com/en-us/azure/sre-agent/user-roles — 「the backend blocks the action with a 403 error」「this behavior isn't yet consistent across all features」
- [3-6] https://learn.microsoft.com/en-us/azure/sre-agent/user-roles — 「az role assignment create」
...
### 説明
Run modes は、エージェントが実行前に承認を求めるかどうかを制御します。権限（Permissions）はリソースへのアクセス可否を制御するもので、Run modes とは別軸です。両方の条件が揃って初めてアクションが実行されます。[5-1]

### 2 つのモード

| モード | 動作 | 推奨用途 |
| --- | --- | --- |
| Review | エージェントがアクションを提案し、ユーザーが承認/拒否 | 本番システム、重要インフラ |
| Autonomous | エージェントが即座に実行し、事後報告 | 非本番環境、信頼できる繰り返しタスク |

> **重要**: Review mode の承認ボタンが表示されるのは Azure インフラ操作（Azure CLI コマンド、ARM 操作等の書き込みアクション）のみです。メール送信、Teams 投稿、外部データソースへのクエリ等は、利用可能なツールと権限に基づいて進行します。[5-2]

### 設定場所
実行モードは **response plan ごと**および **scheduled task ごと**に設定します。エージェントレベルでは直接設定しません。[5-3]
...
エージェントの Settings > Basics にはグローバルモード（既定 Review）がありますが、これは response plan / task で個別設定がない場合のフォールバックです。[5-4]

> **初版からの変更**: 旧稿ではグローバル既定が Review で scheduled task 既定も Review と記載していましたが、最新 docs では response plan・scheduled task ともに既定は **Autonomous** です。[5-3]
...
| エージェント権限あり | モード | 動作 |
| --- | --- | --- |
| あり | Review / Autonomous | そのまま実行 |
| なし | Review / Autonomous | OBO で一時アクセスを要求 |

**書き込みアクション:**

| エージェント権限あり | モード | 動作 |
| --- | --- | --- |
| あり | Review | 同意を求めてから実行 |
| なし | Review | 同意を求め、さらに OBO を要求 |
| あり | Autonomous | そのまま実行 |
| なし | Autonomous | OBO で一時アクセスを要求 |
...
| --- | --- |
| 本番インシデント | Review |
| ステージング/開発インシデント | Autonomous |
| 日次ヘルスチェック | Autonomous |
| コスト・使用量レポート | Autonomous |
| セキュリティアラート | Review |

**Review mode で 2〜4 週間運用し、繰り返し承認しているパターンだけを Autonomous に切り替える**のが推奨されています。[5-7]

### スライド要点
- Autonomous の可否は「incident かどうか」ではなく、「response plan / task 設定」と「エージェントレベルのフォールバック」で決まる。[5-3][5-4]
- Autonomous でも権限不足は消えない。承認フローと RBAC は別軸で残る。[5-5][5-6]
- まずは Review で始め、パターンが安定してから段階的に Autonomous 化する。[5-7]

### 参考（第5章）
- [5-1] https://learn.microsoft.com/en-us/azure/sre-agent/run-modes — 「Run modes control the approval workflow … Permissions control resource access … The agent needs both conditions satisfied to act」
- [5-2] https://learn.microsoft.com/en-us/azure/sre-agent/run-modes — 「Review mode shows Approve and Deny buttons only for Azure infrastructure operations」
- [5-3] https://learn.microsoft.com/en-us/azure/sre-agent/run-modes — 「Set run modes per response plan and per scheduled task」「Default: Autonomous」
- [5-4] https://learn.microsoft.com/en-us/azure/sre-agent/run-modes — 「Settings > Basics shows the agent's global mode … defaults to Review … serves as the fallback」
- [5-5] https://learn.microsoft.com/en-us/azure/sre-agent/run-modes — 「Read-only actions」
- [5-6] https://learn.microsoft.com/en-us/azure/sre-agent/run-modes — 「Write actions」
- [5-7] https://learn.microsoft.com/en-us/azure/sre-agent/run-modes — 「Start with review mode. Observe what the agent recommends for two to four weeks」
...
### 1. アクセス制御の説明を更新する
旧稿の「4レイヤー」は、最新 docs（2026-03-28 更新）では「**3レイヤー**」に変更されています。OBO はエージェント権限レイヤーの一部として整理されました。[3-1][3-2]

### 2. Run modes の既定値を正しく理解する
Response plan・scheduled task の既定は **Autonomous** です。エージェントレベルのグローバル既定は Review ですが、これは個別設定がない場合のフォールバックです。[5-3][5-4]
...
### 6. リファレンス URL の言語を統一する
最新 docs では URL パス構造が変更されたページがあります（例: `roles-permissions-overview` → `user-roles`、`agent-run-modes` → `run-modes`）。リンク切れに注意してください。
...
一方で、権限設計はむしろ細かく整理されており、ユーザー ロール、実行モード、エージェント権限（OBO 含む）の 3 レイヤーを分けて考えないと誤読しやすいです。[3-1][4-5][5-1]
...
SRE Agent の機能をまんべんなく体験していただくための 6 本のシナリオです。Reader 権限のみで安全に始められるものから順に並べています。

### シナリオ × 機能カバレッジマップ

| 機能 | #1 対話型調査 | #2 インシデント自動対応 | #3 定期ヘルスチェック | #4 運用ナレッジ蓄積 | #5 是正アクション | #6 レポート生成 |
| --- | --- | --- | --- | --- | --- | --- |
| Chat 調査 | **◎** | ○ | | | ○ | |
| Built-in クエリ (Log Analytics / Metrics / ARG) | **◎** | ○ | ○ | | ○ | ○ |
| Response plan | | **◎** | | | | |
| Subagent routing | | **◎** | | | ○ | |
| Scheduled tasks | | | **◎** | | | ○ |
...
### シナリオ #1：対話型リソース調査（所要 15 分）

**目的**: Chat の基本操作と built-in クエリ機能を体験する

**前提**: Reader 権限のみで実施可能

**手順**:

1. エージェントに管理対象のリソース一覧を聞く

    ```text
    管理しているサブスクリプションとリソースグループの一覧を教えてください。
    ```

2. 特定のリソース種別を掘り下げる

    ```text
    Web アプリの一覧を表示してください。非推奨のランタイムバージョンで動いているものはありますか？
    ```

3. メトリクスの可視化を依頼する

    ```text
    Web アプリ <APP_NAME> の過去 1 週間の CPU 使用率をグラフで表示してください。
    ```

4. Log Analytics のログを自然言語で検索する

    ```text
    Web アプリ <APP_NAME> で過去 24 時間に発生したエラーの上位 10 件を表示してください。
    ```

**体験できる機能**: Chat 調査、Built-in クエリ（ARG / Metrics / Log Analytics）、Code interpreter による可視化

---

### シナリオ #2：インシデント自動対応の構成とテスト（所要 30 分）

**目的**: Response plan と subagent routing、Run modes の動作を体験する

**前提**: Administrator ロール、Azure Monitor アラートが発火可能な環境

**手順**:

1. **Response plan を作成する** — Incident filters で severity を指定し、対応する subagent を接続する

    - Autonomy level は **Review** に設定する（承認フローを体験するため）

2. **過去インシデントでテストする** — Response plan の Test 機能で過去のアラートを選択し、エージェントの診断プロセスを確認する（テストモードは常に read-only）[7-8]

3. **実際にアラートを発火させる** — テスト用リソースで意図的に障害を起こし（例: App Service のデプロイスロットでエラーを注入[14-1]）、エージェントが自動でスレッドを起こすことを確認する

4. **Review mode の承認フローを体験する** — エージェントが提案する是正アクションに対して Approve / Deny を選択する

5. （オプション）**Connectors で Teams に通知を飛ばす**  — インシデント発生時に Teams チャネルへ通知が来ることを確認する

**体験できる機能**: Response plan（フィルタ + subagent）、Run modes（Review）、Incident platform（Azure Monitor）、Connectors（Teams）、Session insights（スレッド完了後に自動生成）

---

### シナリオ #3：定期ヘルスチェックの自動化（所要 15 分）

**目的**: Scheduled tasks による proactive 運用を体験する

**前提**: Standard User 以上

**手順**:

1. **ヘルスチェック task を作成する**

    ```text
    毎日午前 9 時に、すべての Web アプリとコンテナーアプリのステータスをチェックする
    ヘルスチェックタスクを作成してください。可用性が 99.9% を下回っているアプリがあれば
    レポートしてください。
    ```

2. `Draft the cron for me` で cron 式を自動生成し、`Polish instructions` で指示文を改善する

3. **セキュリティチェック task を作成する**

    ```text
    毎週、アプリケーション <APP_NAME> のセキュリティレビューを実行するタスクを作成してください。
    認証、シークレット、アクセス制御、インフラストラクチャ、依存関係に焦点を当ててください。
    ```

4. Autonomy level を **Autonomous** に設定し、自動実行の結果が事後レポートとして残ることを確認する

**体験できる機能**: Scheduled tasks（作成・cron・Polish instructions）、Run modes（Autonomous）、Built-in クエリ

---

### シナリオ #4：運用ナレッジの蓄積と活用（所要 20 分）

**目的**: Memory system と Knowledge base の動作を体験する
...
1. **手動メモリの登録と検索**
    ...
    ```text
    #remember Web アプリ <APP_NAME> は /api/health というカスタムヘルスエンドポイントを
    使用しています。デフォルトの /health ではなく、必ずこのエンドポイントを最初に確認してください。
    ```
...
2. **Knowledge base に既存 runbook をアップロードする** — `.md` または `.txt` 形式の運用手順書をアップロードし、エージェントが参照できることを確認する

3. **シナリオ #2 で生成された Session insights を確認する** — スレッド完了後 30 分程度で自動生成される session insights の内容（症状・解決手順・根本原因・落とし穴）を確認する[8-7]

4. **蓄積したナレッジを活用した調査を体験する**

    ```text
    Web アプリ <APP_NAME> のレスポンスが遅くなっています。
    過去のインシデントに類似した事例がないか確認してください。
    ```

    エージェントが過去インシデントやアップロード済み runbook を参照して回答することを確認する
...
**体験できる機能**: Memory（#remember / #retrieve / #forget）、Knowledge base、Session insights、過去インシデント検索

---

### シナリオ #5：是正アクションと OBO（所要 20 分）

**目的**: 書き込み操作における Review mode と OBO フォールバックを体験する

**前提**: Administrator ロール、エージェントの Managed ID には Reader のみ付与

**手順**:

1. **Reader 権限のみの状態で是正を依頼する**

    ```text
    Web アプリ <APP_NAME> が非推奨のランタイムバージョンで動作しています。
    最新のサポート対象バージョンに更新してもらえますか？
    ```

2. エージェントが権限不足を検知し、**OBO 承認を求める**ことを確認する — 管理者が OBO を承認すると、一時的にユーザー権限でアクションが実行される[4-5][4-6]

3. **エージェントに Contributor を追加した後、同じ操作を再依頼する** — 今度は OBO なしで Review mode の承認フローのみで実行されることを確認する

4. **Approve / Deny の判断を体験する** — エージェントが提案するアクション（Azure CLI コマンド等）の内容を確認し、Approve する

**体験できる機能**: Run modes（Review）、OBO フォールバック、エージェント権限（RBAC）、書き込みアクション

---

### シナリオ #6：レポート・可視化の自動生成（所要 15 分）

**目的**: Code interpreter と Python tools による分析・レポート生成を体験する
...
1. **インシデントデータの分析を依頼する**
    ...
    ```text
    過去 1 か月に発生したインシデントの CSV ファイルを作成してください。
    データを分析して、カテゴリ別の傾向を表示してください。
    ```

2. **可視化を依頼する**

    ```text
    管理しているすべてのサブスクリプションで、コンテナーアプリ、Web アプリ、
    AKS クラスターの割合を円グラフで表示してください。
    ```

3. **SLA 算出を依頼する**（Python tools が設定済みの場合）

    ```text
    稼働時間 43185 分、ダウンタイム 15 分で SLA を計算してください。
    ```

4. （オプション）**Scheduled task としてレポートを自動化する**

    ```text
    毎週月曜日の午前 8 時に、すべての Web アプリの週次可用性レポートを生成し、
    Teams チャネルにサマリを送信するタスクを作成してください。
    ```

**体験できる機能**: Code interpreter（CSV 生成・可視化・PDF レポート）、Python tools、Scheduled tasks
...
| 4 | #6 レポート・可視化 | ★★☆ | 65 分 |
| 5 | #2 インシデント自動対応 | ★★★ | 95 分 |
| 6 | #5 是正アクションと OBO | ★★★ | 115 分 |

Reader のみで安全に始められる #1 → #4 → #3 を先に体験し、書き込み操作を伴う #2 → #5 は後半で実施する流れです。半日あれば全シナリオを通せます。

### 参考（第14章）
- [14-1] https://learn.microsoft.com/azure/sre-agent/troubleshoot-azure-app-service — 「Tutorial: Troubleshoot an App Service app by using Azure SRE Agent」
