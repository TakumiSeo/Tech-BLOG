...
- Azure Managed Grafana はデータを「受け取って保管する」よりも、データソースを「クエリして表示する」ため、接続成立条件は次の 2 点です。
	- RBAC: Grafana の ID に必要ロール（例: Monitoring Reader / Monitoring Data Reader）を付与
	- Network: 必要なら managed private endpoint（Private Link）を使ってデータソースへ到達
	- 参照:
