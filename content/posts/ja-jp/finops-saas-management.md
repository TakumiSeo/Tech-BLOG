...
| **ユーザー** | Basic（無料）、Core（$49/月）、Full Platform（$349/月〜、エディション依存） | $0〜$349/月 |
...
FOCUS（FinOps Open Cost and Usage Specification）1.2 では、SaaS / PaaS の課金データもパブリッククラウドと同じスキーマに統合されました。これにより、**仮想通貨（Virtual Currency）** — Snowflake の Credit、New Relic の CCU 等 — を一貫したフォーマットで管理できます[^5]。

### 各ツールの FOCUS 対応状況

| ツール | FOCUS 対応 | データ取得方法 | 備考 |
| --- | --- | --- | --- |
| **Azure** | ◎ ネイティブ対応 | Cost Management の FOCUS エクスポート | Microsoft が FOCUS に直接貢献 |
| **New Relic** | △ 要変換 | NerdGraph API + 請求書 | データインジェスト量は API で取得可能。FOCUS フォーマットへの変換が必要 |
| **PagerDuty** | × 非対応 | 請求書 + REST API | ユーザー数ベースのため変換は比較的単純 |
| **Datadog** | △ 要変換 | Usage API + 請求書 | ホスト別・サービス別のデータは API で取得可能 |
| **Snowflake** | ○ 部分対応 | Account Usage スキーマ | クレジット消費の詳細データが取得可能。FOCUS 1.2 で仮想通貨に対応 |
| **GitHub** | △ 要変換 | Billing API + Actions 使用量 API | Actions 分・Packages 容量は API で取得可能 |

> **ベンダーへの確認推奨事項**: FOCUS 形式の請求データエクスポートに今後対応する予定があるか、各ベンダーに確認することを推奨する。対応が進むほど、手動変換の工数が削減できる。

### FOCUS 1.2 の SaaS 対応カラム例（Snowflake の場合）
...
graph LR
    ...
    NR[New Relic API]:::saas --> ETL[ETL / データ変換]:::azure
    PD[PagerDuty API]:::saas --> ETL
    DD[Datadog API]:::saas --> ETL
    SF[Snowflake Usage]:::saas --> ETL
    CM[Azure Cost Management FOCUS Export]:::azure --> ETL

    ETL --> DL[(Data Lake / SQL DB)]:::store
    DL --> PBI[Power BI ダッシュボード]:::viz
...
理想像としては、Power BI 上で複数の SaaS／ツールのコストをまとめて確認できる状態を目指す。これにより、コスト配賦方式の成熟度を高めるとともに、分析や説明の手間を減らしていく。
